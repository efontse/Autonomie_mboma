...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
    ...
    @media (max-width:900px) { .resume { grid-template-columns:1fr 1fr; } .row-prog { display:none; } }
...
<div class="page-wrap">
  ...
  @if($inscriptions->hasPages())
    <div class="pagination">
      <a class="pg {{ $inscriptions->onFirstPage() ? 'off' : '' }}"
         href="{{ $inscriptions->previousPageUrl() }}">← Précédent</a>
      @foreach($inscriptions->getUrlRange(1, $inscriptions->lastPage()) as $page => $url)
        <a class="pg {{ $inscriptions->currentPage() === $page ? 'actif' : '' }}"
           href="{{ $url }}">{{ $page }}</a>
      @endforeach
      <a class="pg {{ !$inscriptions->hasMorePages() ? 'off' : '' }}"
         href="{{ $inscriptions->nextPageUrl() }}">Suivant →</a>
    </div>
  @endif

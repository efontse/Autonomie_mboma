...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
    ...
    /* Progress bar pour auto-fermeture */
    .toast-progress {
      position: absolute;
      bottom: 0;
      left: 0;
...
<body>
  ...
  <script>
    ...
    // Fonction pour afficher une carte toast
    ...
    document.querySelectorAll('.form-inscription').forEach(form => {
      form.addEventListener('submit', function(e) {
        ...
        .then(data => {
          if (data.success) {
            showToast('success', 'Inscription réussie !', data.message || 'Vous êtes maintenant inscrit à cette formation.');
            btn.textContent = 'Inscrit';
            btn.classList.add('btn-success');
            btn.disabled = true;
          } else {
            showToast('error', 'Erreur d\'inscription', data.message || 'Une erreur est survenue lors de l\'inscription.');
        ...
        .catch(error => {
          ...
          showToast('error', 'Erreur de connexion', 'Impossible de se connecter au serveur. Veuillez réessayer.');
          btn.textContent = 'S\'inscrire';
          btn.disabled = false;
        });
      });

...
<head>
  ...
  <title>Modifier formation — Admin Mboma</title>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
    ...
    @media (max-width:900px) { .page-wrap { margin-left:0; padding:1.5rem; } .form-row { grid-template-columns:1fr; } .sidebar { transform:translateX(-100%); } .sidebar.ouvert { transform:translateX(0); } }
    /* Bouton menu mobile */
      ...
      <form action="{{ route('admin.formations.update', $formation) }}" method="POST">
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <select name="type" required>
              <option value="video" {{ $formation->type == 'video' ? 'selected' : '' }}>Vidéo</option>
              <option value="document" {{ $formation->type == 'document' ? 'selected' : '' }}>Document</option>
              <option value="article" {{ $formation->type == 'article' ? 'selected' : '' }}>Article</option>
              <option value="mixte" {{ $formation->type == 'mixte' ? 'selected' : '' }}>Mixte</option>
            </select>
          </div>
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <input type="number" name="duree_minutes" value="{{ $formation->duree_minutes }}" required min="1">
          </div>

          <div class="form-group">
            <label>URL Vidéo (YouTube)</label>
            <input type="url" name="video_url" value="{{ $formation->video_url ?? '' }}" placeholder="https://youtube.com/...">
          </div>
        </div>

        <div class="form-group">
          <label>URL Image</label>
          <input type="url" name="image_url" value="{{ $formation->image_url ?? '' }}" placeholder="https://...">
        </div>
        ...
        <div class="form-group">
          ...
          <textarea name="contenu" required style="min-height:300px;">{{ $formation->contenu }}</textarea>
        </div>

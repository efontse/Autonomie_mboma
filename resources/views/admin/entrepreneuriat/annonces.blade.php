...
<head>
  ...
  <title>Annonces — Admin Mboma</title>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
    ...
    .stat-card .value.actif { color:#059669; }
    .stat-card .value.inactif { color:#6B7280; }
...
<body>
  ...
  <div class="page-wrap">
    ...
    @php
    $totalAnnonces = $annonces->total();
    $actives = \App\Models\Annonce::where('statut', 'actif')->count();
    $inactives = \App\Models\Annonce::where('statut', 'inactif')->count();
    @endphp

    <div class="stats-row">
      <div class="stat-card">
        <h3>Total</h3>
        <div class="value">{{ $totalAnnonces }}</div>
      </div>
      <div class="stat-card">
        <h3>Actives</h3>
        <div class="value actif">{{ $actives }}</div>
      </div>
      <div class="stat-card">
        <h3>Inactives</h3>
        <div class="value inactif">{{ $inactives }}</div>
      </div>
    </div>
      ...
      <table>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td><span class="badge badge-{{ $annonce->type }}">{{ ucfirst($annonce->type) }}</span></td>
            <td>{{ $annonce->secteur }}</td>
            <td>{{ $annonce->prix ? number_format($annonce->prix, 0, ',', ' ') . ' €' : '-' }}</td>
            <td>{{ $annonce->user->prenom ?? '' }} {{ $annonce->user->nom ?? '' }}</td>
            <td><span class="badge badge-{{ $annonce->statut }}">{{ ucfirst($annonce->statut) }}</span></td>
            <td class="actions">
              <a href="{{ route('admin.entrepreneuriat.annonces.show', $annonce) }}" class="btn-action">Voir</a>
              @if($annonce->statut === 'actif')
              <form action="{{ route('admin.entrepreneuriat.annonces.desactiver', $annonce) }}" method="POST" style="display:inline">
                @csrf @method('PATCH')
                <button type="submit" class="btn-action" title="Désactiver">⏸</button>
              </form>
              @else
              <form action="{{ route('admin.entrepreneuriat.annonces.activer', $annonce) }}" method="POST" style="display:inline">
                @csrf @method('PATCH')
                <button type="submit" class="btn-action btn-success" title="Activer">▶</button>
              </form>
              @endif
          ...
          @empty
          <tr>
            <td colspan="7" style="text-align:center; padding:2rem;">Aucune annonce trouvée</td>
          </tr>
          @endforelse
        </tbody>
      </table>

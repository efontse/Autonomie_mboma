...
<body>
  ...
  <script>
    ...
    new Chart(formationsCtx, {
      type: 'bar',
      data: {
        labels: {!! json_encode($formationsPopulaires->pluck('titre')->map(function($t) { return strlen($t) > 25 ? substr($t, 0, 25) . '...' : $t; })) !!},
        datasets: [{
          label: 'Nombre d\'inscriptions',
          data: {!! json_encode($formationsPopulaires->pluck('inscriptions_count')) !!},
    ...
    new Chart(categoriesCtx, {
      type: 'doughnut',
      data: {
        labels: {!! json_encode($inscriptionsParCategorie->pluck('nom')) !!},
        datasets: [{
          data: {!! json_encode($inscriptionsParCategorie->pluck('inscriptions')) !!},
          backgroundColor: [
    ...
    new Chart(moisCtx, {
      type: 'line',
      data: {
        labels: {!! json_encode($inscriptionsParMois->pluck('mois')->map(function($m) { return \Carbon\Carbon::parse($m . '-01')->format('M Y'); })) !!},
        datasets: [{
          label: 'Inscriptions',
